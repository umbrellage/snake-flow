...
public interface JulietFlowClient {
    ...
    AjaxResult<GraphVO> templateGraph(@RequestParam(value = "id", required = false) Long id,
                                      @RequestParam(value = "code", required = false, defaultValue = "flow_dev_process_ty") String code);
    ...
    @ApiOperation("删除")
    @PostMapping("/bpm/delete")
    AjaxResult<Void> delete(@RequestBody InvalidDTO dto);
    ...
    @ApiOperation("规则自动流转")
    @PostMapping("/bpm/flow/automate")
    AjaxResult<Void> flowAutomate(@RequestParam Long flowId,
        @RequestBody Map<String, Object> automateParam);
    ...
    AjaxResult<List<HistoricTaskInstance>> rollback(@RequestBody RollbackDTO dto);
}
